...
contract UpsideAcademyLending {
    ...
    function withdraw(address _asset, uint256 _amount) external {
        ...
        if (_asset == address(0)) {
            // 이더를 출금한 후 필요한 담보 <= balance가 망가지지 않도록.
            ...
            uint256 maxDepositable = getAccruedSupplyAmount(msg.sender); // 이자 한번 더 계산.
    ...
    function borrow(address _asset, uint256 _amount) external {
        ...
        userBalances[msg.sender].borrowedAsset =
            userBalances[msg.sender].borrowedAsset * pow(INTEREST_RATE, borrowedPeriod) / DECIMAL; // 이자 계산
    ...
    function updateUSDC() internal {
        ...
        for (uint256 i = 0; i < suppliedUsers.length; i++) {
            // updates all users' interest at once
