...
contract DreamAcademyLending is DreamOracle {
    ...
    function deposit(address token, uint256 amount) external payable {
        ...
        if (token == address(0x0)) {
            ...
            require(usdc.allowance(msg.sender, address(this)) >= amount, "ERC20: insufficient allowance");
            require(usdc.balanceOf(msg.sender) >= amount, "ERC20: transfer amount exceeds balance");
    ...
    function borrow(address token, uint256 amount) external {
        ...
        _borrowUSDCs[msg.sender].amount =
            _calculateInterest(_borrowUSDCs[msg.sender].amount, _borrowUSDCs[msg.sender].blockNumber, INTEREST_RATE);
    ...
    function liquidate(address user, address tokenAddress, uint256 amount) external {
        ...
        _borrowUSDCs[user].amount =
            _calculateInterest(_borrowUSDCs[user].amount, _borrowUSDCs[user].blockNumber, INTEREST_RATE);

        _borrowUSDCs[user].blockNumber = block.number;

        uint256 borrowValue = (_borrowUSDCs[user].amount * usdcPrice) / ethPrice;

        uint256 remainingCollateralValue = ((_depositETHs[user]) * LT) / 100;
        require(remainingCollateralValue < borrowValue, "Sufficient collateral");
        require(amount == (_borrowUSDCs[user].amount * 1) / 4, "Invalid amount");
    ...
    function withdraw(address tokenAddress, uint256 amount) external {
        ...
        if (tokenAddress == address(0x0)) {
            ...
            uint256 borrowValue = (_borrowUSDCs[msg.sender].amount * usdcPrice) / ethPrice;

            uint256 remainingCollateralValue = ((_depositETHs[msg.sender] - amount) * LT) / 100;

            require(remainingCollateralValue >= borrowValue, "Insufficient collateral");

            _depositETHs[msg.sender] -= amount;
            payable(msg.sender).call{value: amount}("");
        } else {
            uint256 ableToWithdraw = getAccruedSupplyAmount(address(usdc));
            require(usdc.balanceOf(address(this)) >= amount && ableToWithdraw >= amount, "Insufficient funds");

            if (_depositUSDCs[msg.sender].amount < amount) {
                _depositUSDCs[msg.sender].interest -= amount - _depositUSDCs[msg.sender].amount;
    ...
    function getAccruedSupplyAmount(address token) public updateDepositInterest returns (uint256) {
        if (token == address(0x0)) {
            return _depositETHs[msg.sender];
        } else {
            console.logUint((_depositUSDCs[msg.sender].amount + _depositUSDCs[msg.sender].interest) / 1e18);
            return _depositUSDCs[msg.sender].amount + _depositUSDCs[msg.sender].interest;
        }
    }

    function _getMaxBorrow(uint256 collateral, uint256 usdcPrice, uint256 ethPrice) internal view returns (uint256) {
        uint256 currntCollateralValue = (collateral * ethPrice) / usdcPrice;
        return (currntCollateralValue * LTV) / 100;
    }

    function _calculateInterest(uint256 amount, uint256 blockNumber, uint256 annualInterestRate)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 totalBorrow = DSMath.rmul(amount, DSMath.rpow(annualInterestRate, timeInSeconds));
    ...
    modifier updateDepositInterest() {
        ...
        uint256 currentBorrowedUSDC = _calculateInterest(totalBorrowedUSDCs, lastUpdateInterestBlock, INTEREST_RATE);

        uint256 interest = currentBorrowedUSDC - beforeBorrowedUSDC;

        for (uint256 i = 0; i < lenders.length; i++) {
            address user = lenders[i];
            _depositUSDCs[user].interest += (interest * _depositUSDCs[user].amount) / totalUSDC;
        }
